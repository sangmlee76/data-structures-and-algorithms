...
const isNum = (input) => {
  // Solution code here...
  const lookForNumbers = /\d/g;
  return lookForNumbers.test(input);
};
...
const isCapitalized = (str) => {
  // Solution code here...
  // const getCapitalizedWords = /[A-Z]\w+/g;
  const getCapitalizedWords = /[A-Z][a-z]+/g;
  const capitalizedWordsList = str.match(getCapitalizedWords);
  if (capitalizedWordsList){
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  const outputArray = [];
  const targetCitiesRegex = /^[A-J][a-z]+.+/g;
  arr.forEach((value, index) => {
    let checkCity = arr[index].match(targetCitiesRegex);
    if(checkCity){
      outputArray.push(checkCity[0]);
    }
  });
  return outputArray;
};

//Note: Worked together with Seamus Brown on this problem
